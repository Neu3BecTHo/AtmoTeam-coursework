...
<div class="post-card" data-post-id="<?= $post->id ?>">
    <div class="post-header">
        <div class="post-author">
            <a href="<?= Url::to(['profile/view', 'id' => $post->user_id]) ?>" class="author-avatar-link">
                <img class="author-avatar" src="<?= Html::encode($avatar) ?>" alt="Аватар <?= $username ?>">
            </a>
        ...
        <?php if (!Yii::$app->user->isGuest && Yii::$app->user->id == $post->user_id): ?>
            <button class="btn-delete-post" onclick="deletePost(<?= $post->id ?>)" title="Удалить пост">🗑️</button>
        <?php endif; ?>
    </div>
        ...
        <div class="post-image <?= count($imageUrls) > 1 ? 'multiple' : '' ?>">
            <?php foreach ($imageUrls as $index => $imageUrl): ?>
                <div class="post-image-item" data-image-index="<?= $index ?>" data-image-url="<?= Html::encode($imageUrl) ?>">
                    <img src="<?= Html::encode($imageUrl) ?>" alt="Изображение поста <?= $index + 1 ?>" class="clickable-image" loading="lazy" onclick="openImageFullscreen('<?= Html::encode($imageUrl) ?>', <?= count($imageUrls) ?>, <?= $index ?>)">
                </div>
            <?php endforeach; ?>
        </div>
    <?php endif; ?>
    ...
    <div class="post-actions-bar">
        ...
        <button class="post-action btn-comment-toggle" onclick="openPostModal(<?= $post->id ?>)" title="Комментарии">
            💬 <span class="action-count"><?= $commentsCount ?></span>
        </button>
        <button class="post-action btn-save <?= $isSaved ? 'saved' : '' ?>" data-post-id="<?= $post->id ?>" title="<?= $isSaved ? 'Удалить из сохранённых' : 'Сохранить' ?>">
            <span class="action-icon"><?= $isSaved ? '🔖' : '📌' ?></span>
        </button>
        <button class="post-action btn-repost <?= $isReposted ? 'reposted' : '' ?>" data-post-id="<?= $post->id ?>" title="<?= $isReposted ? 'Отменить репост' : 'Сделать репост' ?>">
