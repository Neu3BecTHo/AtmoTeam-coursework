  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        let mime = "image/webp",
          quality = MSG_WEBP_QUALITY;
        if (!canvas.toBlob) {
          mime = "image/jpeg";
          quality = MSG_JPEG_QUALITY;
        }
        canvas.toBlob(
          (blob) => {
...
async function handleImageSelection() {
  ...
  try {
    ...
    if (preview) {
      preview.innerHTML = selectedImageFiles
        .map(
          (_, i) => `
  ...
  try {
    ...
    if (result.success) {
      const sentMessages = JSON.parse(
        localStorage.getItem("sent_messages") || "{}",
      );
...
async function sendImageMessage(content) {
  ...
  try {
    const res = await fetch("/api/message/upload-images", {
      method: "POST",
      body: formData,
    });
...
async function markAsRead(senderId) {
  try {
    await postJson("/api/message/mark-read", { sender_id: senderId });
  } catch {}
}
...
async function pollMessages() {
  ...
  try {
    ...
    const res = await fetch(
      `/api/message/get-dialogue/${receiverId}?_=${Date.now()}`,
    );
    ...
    if (data.success && Array.isArray(data.messages)) {
      ...
      // 1. Получаем ID сообщений с сервера
      const serverMessageIds = new Set(
        data.messages.map((msg) => msg.id).filter((id) => id),
      );

      // 2. Удаляем из DOM сообщения, которых нет на сервере
      const messagesToRemove = [];
      container.querySelectorAll(".message").forEach((msgEl) => {
        const id = msgEl.dataset.messageId;
        if (id && !serverMessageIds.has(parseInt(id))) {
          messagesToRemove.push(msgEl);
        }
      });

      messagesToRemove.forEach((msgEl) => {
      ...
      if (hasNew || messagesToRemove.length > 0) {
        scrollToBottom();
      }
    }
  } catch (error) {
...
function formatDialogueTime(timestamp) {
  ...
  if (diff < 86400)
    return `${Math.floor(diff / 3600)} ${window.t("hours_ago")}`;
  if (diff < 2592000)
    return `${Math.floor(diff / 86400)} ${window.t("days_ago")}`;
...
async function updateDialoguesList() {
  ...
  if (path !== "/message" && path !== "/message/index") return;
  try {
    ...
    if (!data.success || !data.dialogues?.length) {
      container.innerHTML =
        ...
        window.t("no_messages") +
        '</p><a href="/search" class="btn-find-users">' +
        window.t("find_users") +
        "</a></div>";
      return;
    }
    ...
    console.error("updateDialoguesList error:", e);
  }
}
...
function viewImageMessage(imageElement, messageElement = null) {
  ...
  const images = messageElement
    ? messageElement.querySelectorAll(".message-image-content, .grouped-image")
    : [];
  const imageUrl = imageElement.src;

  if (images.length > 1) {
    currentMessageImages = Array.from(images).map((img) => img.src);
    currentMessageImageIndex = Array.from(images).findIndex(
...
function openMessageImageFullscreen(imageUrl) {
  const existing = document.querySelector(
    ".message-image-viewer-modal.fullscreen",
  );
  if (existing) existing.remove();
...
function prevMessageImage(e) {
  ...
  currentMessageImageIndex =
    (currentMessageImageIndex - 1 + currentMessageImages.length) %
...
async function deleteMessage(messageId) {
  ...
  const deleteHandler = async () => {
    try {
      const response = await postJson("/api/message/delete", {
  ...
  if (typeof window.showDeleteModal === "function") {
    window.showDeleteModal(window.t("delete_message_question"), deleteHandler);
  } else {
    if (confirm(window.t("delete_message_question"))) {
      await deleteHandler();
    }
  }
}
...
function initMessageHandlers() {
  ...
  setInterval(updateDialoguesList, interval);
